         ...
         <Form ref="formValidate" :model="formCustom"  :label-width="100">
             <Row>
               ...
               <Col :xs="24" :sm="24" :md="24" :lg="12">
                 <div class="">
                   <FormItem label="上电原因:">
                     <Select v-model="forcePowerOffReason">
                         <Option value="1">电量不足</Option>
                         <Option value="2">修复车辆</Option>
                     </Select>
                   </FormItem>
                 </div>
               </Col>
    ...
    ok() {
      ...
      let  data = {
        data:JSON.stringify({
          ...
          forcePowerOffReason:this.forcePowerOffReason,
          id:this.formCustom.id
        })
      }
      this.$api.post('/bike/forcepoweron',data,reset => {
          if (reset.codeId === 1) {
