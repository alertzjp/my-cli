       ...
       <Form ref="formCustom" :model="formCustom" :label-width="90">
           <Row>
             ...
             <Col :xs="24" :sm="12" :md="8" :lg="6" :xl="6" :xxl="4">
               ...
               <Col :xs="24" :sm="12" :md="8" :lg="6" :xl="6" :xxl="4">
                  <FormItem label="手机号">
                      <Input type="text" v-model="formCustom.mobile" placeholder="请输入手机号"></Input>
                  </FormItem>
               </Col>
...
<script>
  ...
  mounted() {
    ...
    if(searchList){
      searchList.map(vla=>{
        if(vla.path==this.$route.path){
          ...
          this.timearr=[this.formCustom.beginTime,this.formCustom.endTime]
        }
      })
    }
    this.list();
    ...
    this.$vm.$on("callbackEnter",(e)=>{
      if(_this.$route.path == "/admin/userManage/refund"){
        _this.handleSubmit()
      }
    })
  },
  methods: {
    ...
    list () {
      ...
      let listdata= {
        ...
        beginTime:""+this.formCustom.beginTime,
        endTime: "" + this.formCustom.endTime,
        account: "" + this.formCustom.account
      }
      let _this =  this
      this.$api.get('/rider/balance_refund_list',listdata,reset => {
        if (reset.codeId === 1) {
