     ...
     <div class="tableListForm">
       <Form ref="formCustom" :model="formCustom" :label-width="90">
           <Row>
               ...
               <Col :xs="24" :sm="12" :md="8" :lg="6" :xl="6" :xxl="4">
                  <FormItem label="指令类型">
                    <Select v-model="formCustom.orderType" placeholder="请选择指令类型">
                         <Option value="">全部</Option>
                         <Option value="0">远程升级</Option>
                         <Option value="1">获取锁状态</Option>
...
export default {
  ...
  mounted() {
    ...
    this.$vm.$on("callbackEnter",(e)=>{
      if(_this.$route.path == "/admin/bicycledown/operation/list"){
        _this.handleSubmit()
      }
    })
  },
  methods: {
    ...
    list () {
      ...
      let _this =  this
      this.$api.get('/downOrder/list',listdata,reset => {
         _this.loading = false
        if (reset.codeId === 1) {
