  ...
  <div class="page-component-view">
   <div class="page-component-bd">
       ...
       <Form ref="formCustom" :model="formCustom" :label-width="90">
           <Row>
            <Col :xs="24" :sm="12" :md="8" :lg="6" :xl="6" :xxl="5">
                  <FormItem label="统计时间">
                    <time-component @timeEmit="timeEmit" type="2" :timearr="timearr"></time-component>
                  </FormItem>
               </Col>
    ...
    <div class="tableListOperator">
      <Buttoncomponent title="借还次数每日统计导出"  @emitButton="handleeport" :loading="exportloading" icon='md-arrow-down' v-if="$hasPerm('/statistic/export_rent_day')"></Buttoncomponent>
     </div>
     <Row>
...
export default {
                ...
                let _this = this;
                  return h('div', [
                      h('Button', {
                          ...
                          style: {
                              marginRight: '5px',
                              display:_this.$hasPerm('/new_statistic/rent_day_detail')?"":"none"
                           },
                          on: {
                              click: () => {
  ...
  mounted() {
    ...
    this.$vm.$on("callbackEnter",(e)=>{
      if(_this.$route.path == "/admin/total/rentallday"){
        _this.handleSubmit()
      }
    })
  },
  methods: {
    ...
    totallist () {
      ...
      let _this =  this
      this.$api.get('/new_statistic/rent_sum',listdata,reset => {
        if (reset.codeId === 1) {
           _this.dataers = reset.resData || []
        }
        else {
          _this.$netcode.getApiCode(reset)
       }
    ...
    list () {
      ...
      let _this =  this
      this.$api.get('/new_statistic/bicycle_rent_day_new',listdata,reset => {
        if (reset.codeId === 1) {
