...
export default {
    ...
    return {
      ...
      columns:[
          ...
          },
          {
              ...
              sortable: true,
              render: (h, params) => {
                let str = this.$webapi.gettime('time',(params.row.begin_time)/1000)
                return h('div', [
                    h('span', str)
              ])
           }
          },
          {
              ...
              sortable: true,
              render: (h, params) => {
                let str = this.$webapi.gettime('time',(params.row.end_time)/1000)
                return h('div', [
                    h('span', str)
              ])
           }
  ...
  mounted() {
    this.formCustom.riderId = this.$route.query.id
  ...
  methods: {
    ...
    list () {
      ...
      let listdata = {
        ...
        riderId:this.formCustom.riderId
     }
      let _this =  this
      this.$api.get('/statistic/detail_unable_return',listdata,reset => {
         _this.loading = false
        if (reset.codeId === 1) {
