                ...
                <Row>
                    <Col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" :xxl="24">
                      <FormItem label="片区分维护人:" >                           
                          <maintainm-component @emitmaintainer="emitmaintainerList"></maintainm-component>                          
                       </FormItem>
                    </Col> 
                    <Col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" :xxl="24">
                      <FormItem label="联系方式:" >  
...
export default {
  ...
  methods:{
    ...
    Addarea (Newdata) {          
      ...
      let regionScope =  this.regionScope.map((item)=>{
      ...
      let data = {
          data:JSON.stringify({
            ...
            regionScope:JSON.stringify(Newdata),
          })
      }
    ...
    areaSave () {   
        let _this = this;            
        let regionScope = this.regionScope
        if(this.regionScope && this.regionScope.length<1){
          this.$Message.info("【当前你还没有绘画片区,请你绘画片区】")
          return
        }  
        this.compute(regionScope,reset=>{
          if(!reset){            
            let data = regionScope.map(item=>{
              return [item.lng,item.lat]              
            })           
            _this.Addarea(data)
          }else{             
