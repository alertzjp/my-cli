...
export default {
  ...
  methods:{
    ...
    areaSave () {
      // this.areaSaveupdate()    
        let _this = this; 
        let newItem =_this.AllPolygon.find(item=>item.Uh.extData.id==_this.districtItem.id)
        if(!newItem){
          //console.log(this.regionScope)
          newItem = this.regionScope
        }else(
          newItem = newItem.getPath()
        )
        if(newItem && newItem.length<1){
          this.$Message.info("【未绘制片区，请绘制】")
          return
        }  
        this.compute(newItem,reset=>{
          if(!reset){            
            let data = newItem.map(item=>{
