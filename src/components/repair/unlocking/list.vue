       ...
       <div class="tableListForm">
           <Form ref="formCustom" :model="formCustom" :label-width="90">
               <Row>
                   ...
                   <Col :xs="24" :sm="12" :md="8" :lg="6" :xl="6" :xxl="4">
                       <FormItem label="开锁时间">
                           <DatePicker type="daterange" @on-change="oktime" :value="timearr" placeholder="请选择开锁时间" style="width: 100%"></DatePicker>
                       </FormItem>
                   </Col>
                   <Col :xs="24" :sm="12" :md="8" :lg="6" :xl="6" :xxl="4">
                     <div class="">
...
export default {
    ...
    return {
      ...
      columns: [
          ...
          },
          {
              ...
              minWidth:80,
              render: (h, params) => {
                let str = this.$status.bicycleStatus(params.row.status)
                return h('div', [
                    h('span', str)
              ])
           }
              ...
              render: (h, params) => {
                  return h('div', [
                      h('Button', {
                          ...
                          on: {
                              click: () => {
                                this.$router.push({path:"/admin/repair/unlocking/repairmap?gpsLongitude="+params.row.gpsLongitude+"&gpsLatitude="+params.row.gpsLatitude})
                              }
                          }
                      }, '位置'),
                  ]);
  ...
  mounted() {
    ...
    this.$vm.$on("callbackEnter",(e)=>{
      if(_this.$route.path == "/admin/repair/unlocking/unlocking"){
        _this.handleSubmit()
      }
    })
  },
  methods: {
    emitbikeStatus (e) {
      this.formCustom.status = ""+e.value
    },
    oktime (e) {
      let [x,y] = e
    ...
    list () {
      ...
      let listdata = {
        data:{
          ...
          bikeType:this.formCustom.bikeType,
      ...
      let _this =  this
      this.$api.get('/repairingBicycle/allm06unlocking',listdata,reset => {
        if (reset.codeId === 1) {
