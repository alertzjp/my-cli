     ...
     <Drawer title="" :scrollable="false" placement="left" :closable="false" :styles="styles" v-model="DrawerShow" >
       <div class="mermber-logo">
         ...
         <div class="member-logo-hd">
           <div class="substyle" :class="{adminsub:userTypeName==1}">运营商管理后台<version></version></div>
         </div>
         </div>
       <Scroll :height='muneheight'>
           <Menu v-if="items.length>0" theme="light" width="auto" :theme="theme2" :open-names="opennames"  ref="child" :active-name="activename"  @on-select="changeMenu" accordion>
                 <div class="">
                   <Submenu :name="index" v-for="(item,index) in items" v-if="item.blank!=-1" >
                       <template slot="title">
                           <span v-if="item.icon"><Icon :type="item.icon"></Icon></span>
                           <span v-else><Icon type="ios-navigate"></Icon></span>
                           {{item.name}}
                       </template>
                         <MenuItem :name="(index)+'-'+(itemlistindex)" v-for="(itemlist,itemlistindex) in item.resourcesList" v-if="itemlist.blank!=-1">
...
export default {
  ...
  mounted() {
    ...
    this.$vm.$on("H5open",(e)=>{
      console.log("H5open")
       if(e<1285){
         _this.DrawerShow = !_this.DrawerShow
  ...
  watch:{
     ...
     '$route':function (to, from) {
       let _this = this;
       let data = {
          name:this.gettitle(to?to.fullPath:from.fullPath,to.meta.titleshow?to.meta.title:""),
          show:true,
          url:to?to.fullPath:from.fullPath
        }
       ...
       this.items.forEach((a,index)=>{
         if(a.isShow!=-1){
           a.resourcesList.forEach((b,bindex)=>{
             if((to.matched[1].path + "/")==b.path){
               _this.opennames = [index]
